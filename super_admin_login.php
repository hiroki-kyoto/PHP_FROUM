<?php
	...
	if($legal)
	{
		...
		{
			//check validate code
			$std = $_SESSION['super-admin-login'];
			if($std!=$validate)
			{
			...
			else
			{
				...
				$sql= "SELECT password FROM super_admin WHERE name="."\"".$uname."\"";
				...
				else
				{
					...
					{
						$_SESSION[$uname] = 'super-admin-login';
						header("Location: super_admin.php?sad=".$uname."&type=super-admin-login");	
					}
				}
			}
		}	
...
<html>
	...
	<body>
			<form id="mFormId" class='container' name="mForm" action='super_admin_login.php' onsubmit="return submitForm();">
				...
				<div class='center'><a href="#"><img id="vdimgck" align="absmiddle" onClick="this.src=this.src+'#'" style="cursor: pointer;" title="看不清？点击更换" src="validatecode.php?source=super-admin-login" alt='点击重新加载验证码' /></a></div>
